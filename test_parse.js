...
// extract State and necessary functions
const fakeWindow = `
const window = {};
...
extracted = extracted.split('function loadEBNF')[0]; // get numToIdx, tryMatchMove, etc.
...
let m1 = moves.find(m => move2Str(m) === 'c3-b4');
if (m1) {
    console.log("Found c3-b4");
    s.applyMove(m1);
    let m2 = tryMatchMove(s, 'd6-c5');
    console.log("tryMatchMove for d6-c5:", m2 ? move2Str(m2) : "NULL");
    if (!m2) {
        console.log("Available moves:", s.getMoves().map(move2Str));
    }
} else {
    console.log("Could not find c3-b4");
}
