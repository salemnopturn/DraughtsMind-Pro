// ═══════════════════════════════════════════════════════════════════════════
// book.js — Opening Book Module for DraughtsMind Pro
// Extracted from DraughtsMind Classic.html
// ═══════════════════════════════════════════════════════════════════════════

// Square encoding for compressed char-pair format:
...
// ── Parse PDN algebraic line to array of {from, to} move objects ────────
...
function buildOpeningBook(State) {
    bookMap = new Map();
    const initState = new State();

    // 1) Base BOOK_DATA (compressed format) — synchronous (fast)
    ...
    // 2) BOOK_DATA_EXT + PDN_EXTRA_LINES processed in async batches
    ...
    function processExtBatch() {
        ...
        else { setTimeout(processPDNBatch, 0); }
    }
    ...
    function processPDNBatch() {
        ...
        for (; pdnIdx < end; pdnIdx++) {
            ...
            } catch(e) { /* invalid line — skip */ }
        }
        if (pdnIdx < PDN_EXTRA_LINES.length) setTimeout(processPDNBatch, 0);
    }
    setTimeout(processExtBatch, 50);
}

// ── Probe the book for a move at the current position ─────────────────────
// Uses static evaluation + softmax with adaptive temperature:
//   Opening (>18 pieces): T=14
//   Middle game (>14): T=10
//   Endgame: T=8
function bookProbe(state) {
    ...
    // Evaluate each candidate with static eval after the move
    const scored = candidates.map(m => {
        const s2 = state.clone();
        s2.applyMove(m);
        return { move: m, score: -s2.eval() }; // negated: eval() is relative to next player
    });
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        BOOK_DATA, BOOK_DATA_EXT, PDN_EXTRA_LINES,
        DARK_SQUARES, BOOK_ALPHA, charToSq,
        algToIdx, parsePDNLine,
        bookAddLine, buildOpeningBook, bookProbe,
        getBookMap: () => bookMap,
        setBookMap: (m) => { bookMap = m; }
    };
}
